...
class Arthropoda extends Animal {
  ...
  wings() {
    return `They have wings, e.g is ${this.name}`;
  }
  circulatorySystem() {
    return `${this.name}s have an open circulatory system`;
  }
  sensoryOrgans() {
    return `${this.name}s has sensory organs like hairs, antennae, simple and compound eyes, auditory organs, and statocyst`;
  }
}
